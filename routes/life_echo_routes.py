...
def generate_scenario_story(scenario_text, location_profile=None):
    try:
        if not groq_client:
            return {'success': False, 'error': 'GROQ_API_KEY not configured'}

        location_profile = location_profile or {}
        city = location_profile.get('city', 'the local area')
        country = location_profile.get('country', '')
        currency_symbol = location_profile.get('currency_symbol', '$')
        currency_code = location_profile.get('currency_code', 'USD')
        climate_note = location_profile.get('climate_note', 'a typical regional climate')
        transport_examples = location_profile.get('transport_examples', ['local transit', 'taxi'])
        emergency_number = location_profile.get('emergency_number', 'local emergency services')

        transport_hint = ', '.join(transport_examples)

        start_time = time.time()

        prompt = f"""Analyze this real estate scenario: "{scenario_text}"

Location context: {city}, {country}. Climate: {climate_note}.
Local transport options to draw from: {transport_hint}.
...
- [Option 3: e.g., Taxi/Cab - 2-min wait time, {currency_symbol}15-25]
- [Option 4: e.g., Metro/Bus - Nearest station 800m]
...
- Taxi/Cab - Book via app, 3-4 minute arrival time, {currency_symbol}25-40
...
- Taxi/Cab - Available at gate, {currency_symbol}20-35 per trip
...
- Currency: STRICTLY use {currency_symbol} ({currency_code}) ONLY. NEVER use any other currency symbol under any circumstance. Example prices should look like {currency_symbol}25-40, {currency_symbol}200-350
- Use transport options realistic to {city} — draw from: {transport_hint}
- If mentioning emergency services, use {emergency_number}
...
TAGLINE: [Tagline here]"""
        ...
        chat_completion = groq_client.chat.completions.create(
            messages=[
                {
                    "role": "system",
                    "content": f"""You are an expert real estate scenario writer based in {city}, {country}. You create two types of content:
...
CRITICAL CURRENCY RULE: You are writing for {city}, {country}. EVERY price MUST be in {currency_code} ({currency_symbol}). NEVER use AED, $, ₹, £, € or any other currency unless it matches {currency_code} under any circumstance. If you use any currency other than {currency_symbol}/{currency_code}, the response is considered a failure.
...
def generate_scenario():
    """Generate a scenario with story only (no image), localized by client_name"""
    try:
        ...
        if client_name:
            ...
            if location:
                location_profile = location.get('config')
            else:
                logger.warning(f"[SCENARIO] No registered location for '{client_name}', using generic defaults")

        logger.info(f"[SCENARIO] Generating for client='{client_name}': {scenario_text[:100]}...")

        story_result = generate_scenario_story(scenario_text, location_profile)

        if not story_result.get('success'):
            return jsonify({'error': 'Failed to generate story', 'details': story_result.get('error')}), 500
