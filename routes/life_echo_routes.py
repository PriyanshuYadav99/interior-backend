...
from content.scenario_prompts import (
    SCENARIO_USER_TEMPLATE, SCENARIO_SYSTEM_TEMPLATE, render, wrong_currency
)
...
def generate_scenario_story(scenario_text, cfg):
    """
    Generate a scenario story with Groq, using this client's city and currency.
    """
    try:
        if not groq_client:
            return {
        ...
        prompt = render(SCENARIO_USER_TEMPLATE, cfg, scenario_text=scenario_text)
        system_prompt = render(SCENARIO_SYSTEM_TEMPLATE, cfg)
        ...
        for attempt in range(2):
            ...
            if not wrong_currency(response_text, cfg):
                break
            logger.warning(f"[GROQ] Wrong currency in output, retry {attempt + 1}")
        else:
            return {
                'success': False,
                'error': 'Story used the wrong currency. Please try again.'
            }
...
def generate_scenario():
    ...
    try:
        ...
        story_result = generate_scenario_story(scenario_text, cfg)
