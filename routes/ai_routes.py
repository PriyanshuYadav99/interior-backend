...
def build_lead_payload(user_id, supabase):
    ...
    Shared helper — fetches all activity data for a user and
    returns a structured dict ready for the temperature scorer.
    Reused by /lead-intelligence, /lead-temperature, and properties_routes.py.
    """
    user_result = supabase.table('users') \
        .select('*') \
        .eq('id', user_id) \
...
# ============================================================
# SHARED: Temperature scoring (used by the route below AND by
# properties_routes.py / a scheduler job via refresh_lead_temperature)
# ============================================================

def score_lead_temperature(lead_data):
    """
    Calls Groq to score a single lead's payload (from build_lead_payload).
    Returns dict: {score, temperature, reasoning, sales_strategy} or None on failure.
    """
    images = lead_data.get('images', [])
    design_styles = list({img['style'] for img in images if img.get('style')})
    vt = lead_data.get('virtual_tour', {})
    ...
    vt_place_names = [p['place_name'] for p in vt.get('places_viewed', []) if p.get('place_name')]
...
{json.dumps(lead_payload, indent=2)}"""
    ...
    try:
        ...
        logger.error(f"[SCORE LEAD TEMPERATURE] {e}")
        return None


def refresh_lead_temperature(user_id, supabase):
    """
    Scores a lead and PERSISTS the result to users.lead_temperature /
    lead_score / lead_reasoning / lead_sales_strategy / temperature_updated_at.
    Call this from a scheduler job or the /refresh-scores endpoint so property
    cards and lead lists can read cached values instead of hitting Groq live.
    Returns True on success, False otherwise.
    """
    lead_data, error = build_lead_payload(user_id, supabase)
    if error or not lead_data:
        return False

    result = score_lead_temperature(lead_data)
    if not result:
        return False
    ...
    supabase.table('users').update({
        ...
        'lead_reasoning': result.get('reasoning', ''),
        'lead_sales_strategy': result.get('sales_strategy', ''),
...
def get_lead_intelligence(user_id):
    try:
        ...
        json_match = re.search(r'\{.*\}', clean, re.DOTALL)
        if not json_match:
            logger.error(f"[AI INTELLIGENCE] Unparseable response: {response_text[:500]}")
            return jsonify({'error': 'Failed to parse AI response'}), 500
...
def get_lead_temperature(user_id):
    try:
        ...
        # persist so property cards / lead lists get the cached value too
        supabase.table('users').update({
            'lead_temperature': result.get('temperature'),
            'lead_score': result.get('score'),
            'lead_reasoning': result.get('reasoning', ''),
...
# ============================================================
# 4. SEND OUTREACH TO ONE LEAD  (flow 5's "Send AI outreach" button)
# POST /api/ai/send-outreach/<user_id>
# body: { "channel": "whatsapp", "message": "..." }
# ============================================================

@ai_bp.route('/send-outreach/<user_id>', methods=['POST'])
def send_lead_outreach(user_id):
    try:
        from app import supabase
        ...
        ok, err = dispatch_message(u, channel, message)
        ...
        if ok:
            ...
            return jsonify({'success': True, 'message': 'Outreach sent'}), 200

        return jsonify({'success': False, 'error': err}), 502

    except Exception as e:
        logger.error(f"[SEND LEAD OUTREACH ERROR] {e}")
